...
if __name__=="__main__":
    ...
    with concurrent.futures.ProcessPoolExecutor(args.threads) as e:
        for seed in range(1, number_of_seeds+1):
            model = os.path.join(models_dir, 'seed_{}'.format(seed))
            if not os.path.exists(model):
                os.mkdir(model)
            for typ in ['fuzzy', 'normal', 'noclust']:
                mt = os.path.join(model, typ)
                if not os.path.exists(mt):
                    os.mkdir(mt)
                if typ != 'noclust':
                    data_dir = os.path.join(gbdts_data, 'seed_{}'.format(seed))
                else:
                    data_dir = os.path.join(gbdts_data, 'noclust')
